...
def calc_data():
    # Load customer data
    customer_data = pd.read_csv('customer_data.csv')  # Step 1.3: Load the dataset

    # Display the first few rows of the dataset
    print(customer_data.head())

    # Fill missing values and encode categorical columns
    customer_data['tenure'].fillna(customer_data['tenure'].mean(), inplace=True)  # Step 1.4: Fill missing tenure with the mean

    state_encoder = LabelEncoder()
    customer_data['encoded_state'] = state_encoder.fit_transform(customer_data['state'])  # Step 1.5: Encode 'state'

    # Scale numerical columns
    scaler = StandardScaler()
    customer_data[['tenure', 'monthly_charges']] = scaler.fit_transform(customer_data[['tenure', 'monthly_charges']])  # Step 1.6: Scale numerical columns
    ...
    model.fit(x_train, y_train)  # Step 2.1: Train the model

    # Make predictions
    y_pred = model.predict(x_test)  # Step 2.2: Make predictions
    ...
    avg_churn_prob = churn_probabilities.mean()  # Step 3.1: Calculate average churn probability

    # Identify high-risk customers (e.g., those with a churn probability above a threshold)
    high_risk_customers = (churn_probabilities > avg_churn_prob).sum()  # Step 3.2: Identify high-risk customers
    ...
    for state, group in customer_data.groupby('state'):
        ...
        churn_rate, high_risk_count = calculate_churn_and_high_risk(group['churn'], avg_churn_prob)  # Step 3.3
...
def calculate_churn_and_high_risk(churn_series, avg_churn_prob):
    ...
    churn_rate = churn_series.mean()  # Step 3.3: Calculate churn rate
    high_risk_mask = (churn_series > avg_churn_prob)
    high_risk_count = high_risk_mask.sum()  # Step 3.4: Count high-risk customers
